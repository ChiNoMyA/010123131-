...
class Player() :
        ...
        for f in self.Fire:
            if f.pos_fire_x < 800 and f.pos_fire_x > 0:
                ...
                if self.monster2 is not None :
                    if self.monster2.deadMonster(f.pos_fire_x,f.pos_fire_y):
                        f.pos_fire_x += f.vel*1000                
            else:
            ...
            if self.jump_height >= -9:
                ...
                if self.jump_height < 0 :
                    if self.pos_y +64 < self.current_map[index][1] :
                        neg = -1
                        if self.current_map[index][1] - (self.pos_y + 64) < (self.jump_height**2) * 0.5 :
                            self.pos_y -= (self.current_map[index][1] - (self.pos_y + 64)) * neg
                        else :
                            self.pos_y -= (self.jump_height**2) * 0.5 * neg
                        self.jump_height -= 1
                    ...
                    vel = error_block(self.pos_x,self.pos_y,self.gui,'top',(self.jump_height**2) * 0.5 )
                    if  self.pos_y <= self.current_map[head_index][1] + self.current_map[head_index][3] and head_index != -1 :
                        self.jump_height = 0
                        self.pos_y -= (self.jump_height**2) * 0.5 * neg
                        self.jump_height -= 1
                    else :
                        if vel < (self.jump_height**2) * 0.5  :
                            self.pos_y -= vel * neg
                            self.jump_height -= 1
                        else :
                            self.pos_y -= (self.jump_height**2) * 0.5 * neg
    ...
    def checkMap(self):
        ...
        if self.map == 1:
            ...
            if self.fristmap:
                self.monster = Monster(500,600-64,self.map)
                self.monster2 = Monster (700,600-64,self.map)
                self.fristmap = False
            
            if self.monster.pos_x > self.pos_x :
                self.monster.side = -1
                self.monster.pos_x += self.monster.vel*2
            elif self.monster.pos_x < self.pos_x :
                self.monster.side = 1
                self.monster.pos_x += self.monster.vel*2

            if self.monster2.pos_x > 800-64 :
                self.monster2.side = -1
                self.monster2.pos_x += self.monster2.vel
            elif self.monster2.pos_x < 0:
                self.monster2.side = 1
                self.monster2.pos_x += self.monster2.vel
                self.monster2.rage = True
            elif self.monster2.pos_x >= 0 and self.monster.pos_x <= 800:
                self.monster2.pos_x += self.monster2.vel
